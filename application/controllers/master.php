...
class Master extends CI_Controller {
    ...
    public function index(){
        ...
        $this->data['data'] = $this->mastermodel->get_client();
        $this->data['template'] = 'master/index';
    ...
    public function add(){
        ...
        $this->data['title'] = "Add Master";
        $this->data['template'] = "master/addform";
        $this->load->view('layout/index',$this->data);
    ...
    public function insert(){
        ...
        $clientdata= array('name'=>$_POST['name'],
                            ...
                            'pod_id'=>$pod_id,
                            'date'=>time());
        $this->db->insert('master',$clientdata);
        ...
        for($i=0; $i < count($_POST['conn_id']);$i++){
              ...
              $this->db->insert('master_fiber_connection',$conndata);
    ...
    public function details($id){
          ...
          $data =  $this->mastermodel->get_client(array('id' => $id));
          if($data[0]->service_type == 2){

            $data[0]->splitter_info = $this->splittermodel->get_splitter_output(array('output_client_id' => $id));

          }
          $data[0]->connection_info = $this->mastermodel->get_client_connection(array('master_id'=>$id));
          $this->data['data'] = $data;
          $this->data['title'] = 'Master Information';
	        $this->data['template'] = 'master/master_detail';
	        $this->load->view('layout/index',$this->data);
    }

    public function delete($id){
          
    	    $master_info = $this->mastermodel->get_client(array('id'=>$id));  
          $this->db->delete('master',array('id' => $id));
          $data =  $this->mastermodel->get_client_connection(array('master_id' => $id));
          foreach($data as $row){

               $this->db->delete('master_fiber_connection',array('id' =>$row->id));
          ...
          $this->keep_logs($master_info , $data);
    }
    ...
    public function edit($id){
        ...
        $this->data['template'] = "master/edit";
        $this->load->view('layout/index',$this->data);
    }                                                                                                                                                                                                                                          
    ...
    public function update(){
         ...
         $this->update_pod();
         $this->update_client();
         $this->update_client_conn();
         redirect(base_url()."master/details/".$_POST['id']);
    	 
    }

    public function update_pod(){

           $this->mastermodel->update_pod_info($_POST['pod_id'],array('location'=>$_POST['device_location'],'device'=>$_POST['device'],'port'=>$_POST['port']));

    }
    public function update_client(){

          $this->mastermodel->update($_POST['id'],array('name'=>$_POST['name'],'location'=>$_POST['location'],'service_type'=>$_POST['service_type'],'core_type'=>$_POST['core_type'],'longitude'=>$_POST['longitude'],'latittude'=>$_POST['lattitude']));
    }
      ...
      public function update_client_conn(){
         ...
         $client_conn = $this->mastermodel->get_client_connection(array('master_id' => $_POST['id'] ));
         ...
         for($i = 0 ;$i < count($_POST['conn_id']); $i++){
               ...
               if($present !== false){
                    ...
                    $this->mastermodel->update_client_connection($_POST['client_conn_id'][$i],$conndata);
                    ...
                    $conndata =  array('master_id'=>$_POST['id'],'connection_id'=>$_POST['conn_id'][$i],'start_point_id'=>$_POST['start_enclosure'][$i], 'end_point_id'=>$_POST['end_enclosure'][$i],'connection1_core_id'=>$_POST['core_color1'][$i],'connection2_core_id'=>$_POST['core_color2'][$i],'order'=>$i+1);
                    $this->db->insert('master_fiber_connection',$conndata);
    ...
    public function clientgraph(){
         ...
         $client =  $this->mastermodel->get_client();
         foreach($client as $row){

            $row->connection_info = $this->mastermodel->get_client_connection(array('master_id'=>$row->id));
         ...
         foreach($data as $row){
            ...
            foreach($row->core_info as $childrow){
                ...
                $childrow->clientinfo = $this->mastermodel->get_client_connection(array('connection1_core_id'=>$childrow->id));
                if(!empty($childrow->clientinfo)){
                    
                      $childrow->clientinfo[0]->client_name = $this->mastermodel->get_client(array('id'=> $childrow->clientinfo[0]->master_id));
                }
         ...
         $this->data['template'] = 'master/mastergraph';
         $this->load->view('layout/index',$this->data);
